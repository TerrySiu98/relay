...
数据存储：轻量级 SQLite 文件存储（默认 `/var/lib/relay/data.db`），无外部数据库依赖，迁移方便。
...
无论是安装新面板、新节点，还是更新版本，都可以使用此脚本。
脚本会自动识别系统架构、下载最新版 v1.0.0，并配置开机自启服务。

```bash
curl -o relay.sh https://raw.githubusercontent.com/TerrySiu98/relay/main/relay.sh && chmod +x relay.sh && ./relay.sh
```
...
```bash
# 下载并运行
curl -L -o relay https://github.com/TerrySiu98/relay/releases/latest/download/relay-linux-amd64
chmod +x relay
./relay -mode master
```

访问 `http://IP:8888`，默认账号 `admin`，密码 `admin`。
...
```bash
# 例如
./relay -mode agent -name "节点1" -connect "面板IP:9999" -token "面板Token"
```
### 赋予执行权限
```
chmod +x relay
...
- 默认使用标准 TCP/UDP 透传模式，适合绝大多数场景。
- V2Board/SS 等服务端口不强制必须为 `443`，可用随机端口。

## 数据目录说明

- 默认数据目录：`/var/lib/relay`
- 主要文件：`/var/lib/relay/data.db`
- 自定义目录：启动前设置环境变量 `RELAY_DATA_DIR=/your/path`
...
配置文件：
所有配置、规则、日志默认保存在 `/var/lib/relay` 目录（可通过环境变量 `RELAY_DATA_DIR` 自定义）。
⚠️ 注意事项
