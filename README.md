...
## 1.一键安装脚本（方式）
```
curl -o go_relay.sh https://raw.githubusercontent.com/TerrySiu98/relay/master/go_relay.sh && chmod +x go_relay.sh && ./go_relay.sh
```
...
#### Docker命令
```
docker run -d --name relay-master --restart=always --net=host -v $(pwd):/data terrysiu/relay -mode master
```
## 3.编译与安装 (Master端方式)
